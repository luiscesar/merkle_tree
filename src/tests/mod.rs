...
use crate::{generate_tree_root, gnereate_merkle_path};
...
fn test_generate_root_3() {
    ...
    assert!(!generate_tree_root(longTest).is_err());
...
fn test_generate_path_1() {
    let mocked_merkle_paths: Vec<(u8, usize, Vec<H256>)> = vec![
        ...
        (
            ...
            vec![
                H256(to_array(
                    32,
                    hex::decode("11aeafa56c9b34805cc86b1c320c9331672c07e600f0a44317051cfa05a0c296")
                        .unwrap()
                        .as_slice(),
                )),
                H256(to_array(
    ...
    for (n, i, paht) in mocked_merkle_paths {
        ...
        let computed_path = gnereate_merkle_path(&nodes[i].clone(), nodes).unwrap();
